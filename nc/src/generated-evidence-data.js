...
window.WEBINSIGHT.EVIDENCE_REPORTS_RAW = [
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang dari {{PREVIOUS_END}}）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
  ...
  },
  {
    ...
    sql:"SELECT url, REGEXP_REPLACE(url, r'^https?://[^/]+', '') AS path, SUM(impressions) AS imp, SUM(clicks) AS clk, ROUND(SUM(sum_position) / NULLIF(SUM(impressions), 0), 1) AS avg_pos FROM all_gsc_summary WHERE site_name='護理學院' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY url ORDER BY imp DESC LIMIT 20;"
  },
  {
    ...
    sql:"SELECT query, SUM(impressions) AS imp, SUM(gsc_clicks) AS clk, SUM(total_behavior_events) AS behavior_events, COUNT(DISTINCT url) AS pages_count FROM search_behavior_summary WHERE site_name='護理學院' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND query IS NOT NULL AND query != '' GROUP BY query ORDER BY imp DESC LIMIT 20;"
  },
  {
  ...
  },
  {
    ...
    sql:"SELECT 'all_units_summary' AS table_name, MAX(date) AS max_date, COUNT(*) AS recent_rows FROM all_units_summary WHERE site_name='護理學院' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'all_gsc_summary', MAX(data_date), COUNT(*) FROM all_gsc_summary WHERE site_name='護理學院' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'search_behavior_summary', MAX(date), COUNT(*) FROM search_behavior_summary WHERE site_name='護理學院' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}';"
  },
  {
  ...
  },
  {
    ...
    sql:"SELECT '電話' AS type, 'NC 系辦' AS name, '+886-3-8572677' AS value, COUNT(*) AS clicks FROM all_units_summary WHERE site_name='護理學院' AND link_url LIKE '%tel:%' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'Email', '護理學系聯絡信箱', 'nc@tcu.edu.tw', COUNT(*) FROM all_units_summary WHERE site_name='護理學院' AND (link_url LIKE '%mailto:nc%' OR link_text LIKE '%nc%@%') AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}';"
  },
  {
  ...
  },
  {
    ...
    sql:"SELECT 'missing_jsonld' AS issue_type, COUNTIF(REGEXP_CONTAINS(body, r'application/ld\+json')) AS affected_count FROM `project.all_units_summary` WHERE site_name='護理學院' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL /* … */ ;"
  }];
