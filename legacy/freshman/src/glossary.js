const GLOSSARY_DB = {
  ...
  "all_gsc_summary": "這是放所有系所「Google 搜尋曝光與點擊」資料的總表。報告只取慈大新鮮人的部分。",
  "all_unit_summary": "拼字錯誤，正確為 all_units_summary。",
  "all_units_summary": "這是放整個慈濟大學「所有 97 個系所」上網資料的總表。報告只取慈大新鮮人的數字，需要先過濾。",
  ...
  "site": "BigQuery 報表中對一個系所的統稱。注意本報告全程固定 `site_name='慈大新鮮人'`。<br><br>例子：`WHERE site_name='慈大新鮮人'`。",
  "site_academy": "系所隸屬的學院。新生網站隸屬於教務處。",
  "site_id": "Google Analytics 給這個系所的內部編號。",
  "site_link_clicks": "在 Google 搜尋結果裡，點擊了「同一個站的其他連結」的次數。",
  "site_url": "系所首頁的網址，例如新生網站的就是 `https://freshman.tcu.edu.tw/`。",
  "social_platform": "使用者從哪個社群平台來，例如 Facebook、LINE。",
  "source": "流量來源，例如 google、facebook.com、(direct)。",
  "sparkline": "小折線圖：KPI 卡裡的趨勢縮圖。",
  "subdomain": "系所的子網域簡寫，新生網站就是 `freshman`。",
  ...
  "view": "瀏覽。",
  "主題與實體清楚度": "首頁有沒有清楚告訴 Google「這是哪個學校、哪個系」。<br><br>📝 例子：好 = 標題寫「慈濟大學新生網站」；不好 = 只寫「首頁」或「歡迎光臨」。",
  "內容結構與可擷取性": "Google 機器人能不能順利讀懂網站內容、把人話整理成答案。<br><br>📝 例子：好 = 內容有清楚的標題階層（H1、H2、H3）、有列表、有表格；不好 = 全部塞在一張大圖片裡，Google 看不到字。",
  "單位網站管理人": "負責網頁內容的人（標題、聯絡電話、CTA、內容）。",
  "工作階段": "Session — 使用者一次連續瀏覽的多個事件，通常 30 分鐘無活動後結束。<br><br>📝 例子：同學進來看完 5 頁然後離開，這段過程算一個「工作階段」。",
  "技術 GEO": "網站有沒有安裝「結構化資料」標記讓 Google 認識學校與系所、有沒有 og 標籤讓分享時漂亮顯示。<br><br>📝 例子：好 = 安裝 JSON-LD 標記 `\"@type\": \"EducationalOrganization\"`；不好 = 完全沒有結構化資料，Google 只知道它是個「網站」但不知道是「大學系所」。",
  "招生問題回答能力": "學生問「學什麼」「怎麼考」「學費多少」時，網站有沒有答案。<br><br>📝 例子：好 = 有「入學方式」頁面列出 115 學年度的入學管道與名額；不好 = 完全沒有提到如何入學。",
  "證據與可信度": "網站有沒有提供「學生作品、競賽得獎、企業實習」這類證據讓人相信這個系很強。<br><br>📝 例子：好 = 列出「畢業生錄取半導體公司 12 位」「學生競賽獲獎 5 項」；不好 = 完全沒有數字、沒有案例。",
  ...
  "DOM ID": "HTML 元素的 id 屬性。同一頁面內 id 必須唯一。重複的 id 會破壞 JavaScript selector、ARIA 與無障礙工具。新生網站首頁偵測到 0 處重複。",
  "Widget": "頁面中的一個互動小區塊（例如嵌入的 YouTube 影片、地圖、聯絡表單）。新生網站首頁有 0 個 iframe widget。",
  ...
  "parentOrganization": "在 schema.org 結構化資料裡，用來標示某個單位（例如新生網站）隸屬於哪個較大的組織（例如慈濟大學）。這層關係幫助 Google 正確判斷機構階層。",
  ...
  "Hln_*": "範本自動產生的 HTML id 前綴（例如 Hln_a1b2）。同一個頁面上 id 必須唯一，重複的 id 會破壞 JavaScript selector、ARIA 與螢幕閱讀器的可訪問性。新生網站首頁偵測到 0 處這類 id 衝突。",
