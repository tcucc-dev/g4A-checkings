...
// Single source of truth for ALL Freshman evidence records. This file is
...
// Migration note (2026-08-06, Freshman migration): this file replaces the
// legacy 669-line evidence.js (which bundled the EVIDENCE_REPORTS array
// + renderer logic). The 20 EV-* records are preserved with original data;
// date literals are converted to {{...}} placeholders so they auto-update
// with REPORT_DATA.periods. Renderer code moved to evidence-renderer.js.
...
window.WEBINSIGHT.EVIDENCE_REPORTS_RAW = [
  ...
  },
  {
    ...
    sql:"SELECT url, REGEXP_REPLACE(url, r'^https?://[^/]+', '') AS path, SUM(impressions) AS imp, SUM(clicks) AS clk, ROUND(SUM(sum_position) / NULLIF(SUM(impressions), 0), 1) AS avg_pos FROM all_gsc_summary WHERE site_name='慈大新鮮人' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY url ORDER BY imp DESC LIMIT 20;"
  },
  {
    ...
    sql:"SELECT query, SUM(impressions) AS imp, SUM(gsc_clicks) AS clk, SUM(total_behavior_events) AS behavior_events, COUNT(DISTINCT url) AS pages_count FROM search_behavior_summary WHERE site_name='慈大新鮮人' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND query IS NOT NULL AND query != '' GROUP BY query ORDER BY imp DESC LIMIT 20;"
  },
  {
  ...
  },
  {
    ...
    sql:"SELECT 'all_units_summary' AS table_name, MAX(date) AS max_date, COUNT(*) AS recent_rows FROM all_units_summary WHERE site_name='慈大新鮮人' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'all_gsc_summary', MAX(data_date), COUNT(*) FROM all_gsc_summary WHERE site_name='慈大新鮮人' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'search_behavior_summary', MAX(date), COUNT(*) FROM search_behavior_summary WHERE site_name='慈大新鮮人' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}';"
  },
  {
  ...
  },
  {
    ...
    sql:"SELECT '電話' AS type, '新生網站' AS name, '+886-3-8565301' AS value, COUNT(*) AS clicks FROM all_units_summary WHERE site_name='慈大新鮮人' AND link_url LIKE '%tel:%' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'Email', '新生網站聯絡信箱', 'freshman@tcu.edu.tw', COUNT(*) FROM all_units_summary WHERE site_name='慈大新鮮人' AND (link_url LIKE '%mailto:itm%' OR link_text LIKE '%itm%@%') AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}';"
  },
  {
  ...
  },
  {
    ...
    description:"依 page_view 事件彙總的「入口頁 → 第二頁」共同出現次數（{{PREVIOUS_START}} 至 {{PREVIOUS_END}} 期間，5 個完整天）。本期資料以 ROW_NUMBER() OVER (PARTITION BY session ORDER BY event_timestamp) 重建單一工作階段內的時序。Refreshed 2026-08-03T11:18:00Z：本期 150/177 工作階段僅有 1 個 page_view，入口頁和第二頁多為同一頁（self-loop），故右側「入口頁重點轉換」表全為 0。",
  ...
  },
  {
    ...
    sql:"SELECT 'missing_jsonld' AS issue_type, COUNTIF(REGEXP_CONTAINS(body, r'application/ld\+json')) AS affected_count FROM `project.all_units_summary` WHERE site_name='慈大新鮮人' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL /* … */ ;"
  }
];

// ===== Backward-compatible aliases =====
if (window.WEBINSIGHT && window.WEBINSIGHT.EVIDENCE_REPORTS) {
  window.WEBINSIGHT.EVIDENCE_REPORTS = window.WEBINSIGHT.EVIDENCE_REPORTS;
}
