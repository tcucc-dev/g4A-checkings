...
// Single source of truth for ALL WWW evidence records. This file is
...
window.WEBINSIGHT.EVIDENCE_REPORTS_RAW = [
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang dari 2026-07-26）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:99d3d996334fd7c33445e2c95f4931701b31c450725908e7722a0fb5693fa1d3", status:"正常",
    ...
    rows:[
    {week:"07/25",sessions:974,pageviews:1356,users:917,engagement_seconds:0.0,pages_per_session:1.35},
    {week:"07/26",sessions:950,pageviews:1344,users:847,engagement_seconds:0.0,pages_per_session:1.41},
    {week:"07/27",sessions:1660,pageviews:2826,users:1286,engagement_seconds:0.0,pages_per_session:1.67},
    {week:"07/28",sessions:2171,pageviews:3409,users:1780,engagement_seconds:0.0,pages_per_session:1.53},
    {week:"07/29",sessions:1752,pageviews:2838,users:1389,engagement_seconds:0.0,pages_per_session:1.59},
    {week:"07/30",sessions:1724,pageviews:2858,users:1331,engagement_seconds:0.0,pages_per_session:1.62},
    {week:"07/31",sessions:1503,pageviews:2512,users:1169,engagement_seconds:0.0,pages_per_session:1.65}
  ],
    sql:"SELECT week, COUNT(DISTINCT ga_session_id) AS sessions, COUNTIF(event_name='page_view') AS pageviews, COUNT(DISTINCT user_pseudo_id) AS users, ROUND(SAFE_DIVIDE(SUM(engagement_time_msec)/1000.0, COUNT(DISTINCT ga_session_id)), 0) AS engagement_seconds, ROUND(SAFE_DIVIDE(COUNTIF(event_name='page_view'), COUNT(DISTINCT ga_session_id)), 2) AS pages_per_session FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '2026-06-01' AND DATE '{{PREVIOUS_END}}' GROUP BY week ORDER BY week;"
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:16349c6be53aa1f293fcaa3fc7c60622b633cc6cceefec9765875b4f54c0b7d0", status:"正常",
    ...
    rows:[
    {dimension:"來源",name:"直接進站",sessions:4679,users:0,pageviews:0},
    {dimension:"來源",name:"自然搜尋",sessions:4614,users:0,pageviews:0},
    {dimension:"來源",name:"外部推薦",sessions:174,users:0,pageviews:0},
    {dimension:"來源",name:"AI 推薦",sessions:2,users:0,pageviews:0},
    {dimension:"裝置",name:"desktop",sessions:6488,users:0,pageviews:0},
    {dimension:"裝置",name:"mobile",sessions:2888,users:0,pageviews:0},
    {dimension:"裝置",name:"tablet",sessions:93,users:0,pageviews:0},
    {dimension:"國家",name:"Taiwan",sessions:7738,users:0,pageviews:0},
    {dimension:"國家",name:"China",sessions:1342,users:0,pageviews:0},
    {dimension:"國家",name:"Japan",sessions:50,users:0,pageviews:0},
    {dimension:"國家",name:"Indonesia",sessions:44,users:0,pageviews:0},
    {dimension:"國家",name:"United States",sessions:44,users:0,pageviews:0}
  ],
    sql:"SELECT '國家' AS dimension, country AS name, COUNT(DISTINCT ga_session_id) AS sessions, COUNT(DISTINCT user_pseudo_id) AS users, COUNTIF(event_name='page_view') AS pageviews FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY country UNION ALL SELECT '裝置', device_category, COUNT(DISTINCT ga_session_id), COUNT(DISTINCT user_pseudo_id), COUNTIF(event_name='page_view') FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY device_category UNION ALL SELECT '流量來源', CONCAT(COALESCE(source,'(direct)'),'/',COALESCE(medium,'(none)')), COUNT(DISTINCT ga_session_id), COUNT(DISTINCT user_pseudo_id), 0 FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY source, medium;"
  },
  {
    id:"EV-GSC-PAGE-001", title:"GSC 頁面曝光點擊完整報表", sourceType:"BigQuery",
    sourceTable:"all_gsc_summary", queryCode:"GSC-PAGE-EVIDENCE-001",
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:8871e10a4f493fe898ee7488f6b4c8b14b9dedb851fd3035a7d9542e4f5896b3", status:"需改善",
    ...
    rows:[
    {rank:1,path:"/",imp:17439,clk:2565,avg_pos:31.0},
    {rank:2,path:"/p/412-1033-4845.php",imp:5998,clk:556,avg_pos:7.8},
    {rank:3,path:"/p/412-1033-4801.php",imp:5580,clk:127,avg_pos:15.6},
    {rank:4,path:"/p/412-1033-4832.php",imp:4902,clk:77,avg_pos:8.0},
    {rank:5,path:"/p/412-1033-4788.php",imp:1739,clk:26,avg_pos:18.9},
    {rank:6,path:"/p/412-1033-4858.php",imp:850,clk:334,avg_pos:7.4},
    {rank:7,path:"/p/426-1033-2.php?Lang=zh-tw",imp:725,clk:34,avg_pos:12.0},
    {rank:8,path:"/p/412-1033-4783.php",imp:709,clk:5,avg_pos:15.6},
    {rank:9,path:"/p/412-1033-4828.php",imp:670,clk:4,avg_pos:21.1},
    {rank:10,path:"/p/412-1033-4789.php",imp:576,clk:15,avg_pos:22.2},
    {rank:11,path:"/p/404-1033-64315.php",imp:560,clk:10,avg_pos:9.3},
    {rank:12,path:"/var/file/33/1033/img/1954/CDR114.pdf",imp:544,clk:30,avg_pos:9.2},
    {rank:13,path:"/var/file/33/1033/img/1954/CDR115.pdf",imp:533,clk:112,avg_pos:7.6},
    {rank:14,path:"/hcsc/index.html",imp:487,clk:2,avg_pos:71.3},
    {rank:15,path:"/p/412-1033-4834.php",imp:468,clk:2,avg_pos:16.6},
    {rank:16,path:"/p/404-1033-65049.php",imp:395,clk:5,avg_pos:22.0},
    {rank:17,path:"/p/412-1033-4809.php",imp:394,clk:5,avg_pos:29.3},
    {rank:18,path:"/var/file/33/1033/img/1953/826463228.html",imp:382,clk:13,avg_pos:20.7},
    {rank:19,path:"/p/404-1033-64305.php?Lang=zh-tw",imp:348,clk:3,avg_pos:11.9},
    {rank:20,path:"/p/412-1033-4918.php",imp:343,clk:3,avg_pos:9.2}
  ],
    sql:"SELECT url, REGEXP_REPLACE(url, r'^https?://[^/]+', '') AS path, SUM(impressions) AS imp, SUM(clicks) AS clk, ROUND(SUM(sum_position) / NULLIF(SUM(impressions), 0), 1) AS avg_pos FROM all_gsc_summary WHERE site_name='慈濟大學中文版首頁' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY url ORDER BY imp DESC LIMIT 20;"
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:d37407662dc5007b134beaea7a258b5e84032a1aa127ea4824b12aca6f1212e1", status:"注意",
    ...
    rows:[
    {query:"慈濟大學",imp:19964,clk:2515,behavior_events:57631,pages_count:25,users:9380},
    {query:"慈濟",imp:3774,clk:8,behavior_events:43924,pages_count:8,users:6891},
    {query:"tcu",imp:1756,clk:218,behavior_events:54548,pages_count:13,users:8830},
    {query:"慈大",imp:1487,clk:240,behavior_events:60543,pages_count:17,users:9804},
    {query:"慈濟科技大學",imp:684,clk:36,behavior_events:50351,pages_count:7,users:8094},
    {query:"慈濟大學建國校區",imp:655,clk:30,behavior_events:52422,pages_count:11,users:8446},
    {query:"慈濟大學行事曆",imp:641,clk:157,behavior_events:20951,pages_count:6,users:3260},
    {query:"慈濟大學校務系統",imp:501,clk:4,behavior_events:45353,pages_count:7,users:7183},
    {query:"交通安全",imp:432,clk:0,behavior_events:0,pages_count:1,users:0},
    {query:"慈濟大學學生資訊系統",imp:415,clk:2,behavior_events:44091,pages_count:4,users:6942},
    {query:"花蓮慈濟大學",imp:410,clk:27,behavior_events:55049,pages_count:8,users:8927},
    {query:"tzu chi university",imp:390,clk:12,behavior_events:46507,pages_count:4,users:7387},
    {query:"site:www.tcu.edu.tw",imp:380,clk:0,behavior_events:20569,pages_count:188,users:3296},
    {query:"花蓮大學",imp:289,clk:2,behavior_events:40101,pages_count:4,users:6241},
    {query:"慈濟大學圖書館",imp:257,clk:0,behavior_events:30628,pages_count:4,users:4742},
    {query:"花蓮慈濟",imp:255,clk:1,behavior_events:33164,pages_count:3,users:5151},
    {query:"tcu 大學",imp:217,clk:5,behavior_events:51142,pages_count:6,users:8236},
    {query:"慈济大学",imp:202,clk:24,behavior_events:54695,pages_count:7,users:8867},
    {query:"慈大學資",imp:195,clk:0,behavior_events:39953,pages_count:7,users:6202},
    {query:"慈濟技術學院",imp:190,clk:6,behavior_events:48382,pages_count:9,users:7754}
  ],
    sql:"SELECT query, SUM(impressions) AS imp, SUM(gsc_clicks) AS clk, SUM(total_behavior_events) AS behavior_events, COUNT(DISTINCT url) AS pages_count FROM search_behavior_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND query IS NOT NULL AND query != '' GROUP BY query ORDER BY imp DESC LIMIT 20;"
  },
  {
  ...
  },
  {
    ...
    sql:"SELECT 'all_units_summary' AS table_name, MAX(date) AS max_date, COUNT(*) AS recent_rows FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'all_gsc_summary', MAX(data_date), COUNT(*) FROM all_gsc_summary WHERE site_name='慈濟大學中文版首頁' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'search_behavior_summary', MAX(date), COUNT(*) FROM search_behavior_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}';"
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:308f12a94db6cf84ac6d53ee594e236da0576149aaba934cd14d46500eb44b96", status:"查詢可用",
    ...
    rows:[
    {week:"07/25",course_pv:3,faculty_pv:0,feature_pv:4},
    {week:"07/26",course_pv:1,faculty_pv:0,feature_pv:3},
    {week:"07/27",course_pv:1,faculty_pv:0,feature_pv:1},
    {week:"07/28",course_pv:0,faculty_pv:0,feature_pv:3},
    {week:"07/29",course_pv:0,faculty_pv:0,feature_pv:7},
    {week:"07/30",course_pv:2,faculty_pv:0,feature_pv:13},
    {week:"07/31",course_pv:2,faculty_pv:0,feature_pv:1}
  ],
    sql:"SELECT FORMAT_DATE('%m/%d', DATE_ADD(DATE_TRUNC(date, WEEK(MONDAY)), INTERVAL 6 DAY)) AS week, SUM(CASE WHEN page_title LIKE '%課程地圖%' OR page_title LIKE '%課程規劃%' OR page_title LIKE '%碩士班專區%' THEN 1 ELSE 0 END) AS course_pv, SUM(CASE WHEN page_title LIKE '%師資陣容%' THEN 1 ELSE 0 END) AS faculty_pv, SUM(CASE WHEN page_title LIKE '%本系特色%' OR page_title LIKE '%業界實習%' OR page_title LIKE '%實習流程%' OR page_title LIKE '%實務專題%' THEN 1 ELSE 0 END) AS feature_pv FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '2026-06-01' AND DATE '{{PREVIOUS_END}}' AND page_title != '' GROUP BY week ORDER BY week;"
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:121f23e6a7d8ba14b78bf9f5e86098ca3a29f19c58e9e5316580f3f7758eeb44", status:"資料正常",
    description:"從 TCU 網頁 HTML 與 GA4 連結點擊目標解析出的聯絡電話、Email、LINE 群組、招生窗口。",
    filterKeys:["type"], chart:{type:"bar",xKey:"name",series:[["clicks","近一週點擊次數"]]},
    columns:[["type","類別"],["name","名稱"],["value","值／連結"],["clicks","近一週點擊次數"]],
    rows:[
    {type:"other",name:"",value:"https://admin.tcu.edu.tw/TCUstweb/stlogin.php",clicks:1087},
    {type:"other",name:"學生",value:"https://www.tcu.edu.tw/p/412-1033-4845.php",clicks:1020},
    {type:"other",name:"",value:"https://freshman.tcu.edu.tw/",clicks:391},
    {type:"other",name:"115學年度行事曆",value:"https://www.tcu.edu.tw/p/412-1033-4858.php?Lang=zh-tw#cmb_95_1",clicks:352},
    {type:"other",name:"115學年度行事曆",value:"https://www.tcu.edu.tw/p/412-1033-4858.php#cmb_95_1",clicks:336},
    {type:"other",name:"校內公告查詢",value:"https://info.tcu.edu.tw/hot_news/news_search.asp",clicks:322},
    {type:"other",name:"115學年度行事曆(PDF)",value:"https://www.tcu.edu.tw/var/file/33/1033/img/1954/CDR115.pdf",clicks:281},
    {type:"other",name:"",value:"https://moodle.tcu.edu.tw/login/index.php",clicks:203},
    {type:"other",name:"行事曆",value:"https://www.tcu.edu.tw/p/412-1033-4858.php?Lang=zh-tw",clicks:175},
    {type:"other",name:"人力資源處",value:"https://personnel.tcu.edu.tw/",clicks:173},
    {type:"other",name:"",value:"https://www.tcu.edu.tw/",clicks:165},
    {type:"other",name:"教務處",value:"https://academic.tcu.edu.tw/",clicks:157},
    {type:"other",name:"總務處",value:"https://gm.tcu.edu.tw/",clicks:127},
    {type:"other",name:"學生事務處",value:"https://sao.tcu.edu.tw/",clicks:114},
    {type:"other",name:"",value:"https://admin.tcu.edu.tw/st/stpaylogin.php",clicks:100},
    {type:"other",name:"閱讀更多",value:"https://tcublog.tcu.edu.tw/",clicks:96},
    {type:"other",name:"",value:"https://sao.tcu.edu.tw/",clicks:81},
    {type:"other",name:"",value:"http://gms.tcu.edu.tw/",clicks:81},
    {type:"other",name:"慈濟大學圖書館",value:"https://lib.tcu.edu.tw/",clicks:80},
    {type:"other",name:"",value:"https://www.tcu.edu.tw/p/412-1033-4846.php?Lang=zh-tw",clicks:80}
  ],
    sql:"SELECT '電話' AS type, 'TCU 系辦' AS name, '+886-3-8572677' AS value, COUNT(*) AS clicks FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND link_url LIKE '%tel:%' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL SELECT 'Email', '資管系聯絡信箱', 'www@tcu.edu.tw', COUNT(*) FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND (link_url LIKE '%mailto:www%' OR link_text LIKE '%www%@%') AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}';"
  },
  {
  ...
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:61f6d16a7c296f7e3f5b4b847b343094b4c6d54ad7c35c5247ed1ce1d2e74580", status:"需改善",
    ...
    rows:[
    {query:"site:www.tcu.edu.tw", imp:400, clk:0, ctr:0, position:5, recommendation:"需建立專門 landing page 或加入系名"},
    {query:"site:www.tcust.edu.tw", imp:132, clk:0, ctr:0, position:5, recommendation:"需建立專門 landing page 或加入系名"},
    ...
    {query:"www", imp:47, clk:0, ctr:0, position:5, recommendation:"需建立專門 landing page 或加入系名"},
  ...
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:977b281f83e19986a471b3f9153d9e0969920485b9db27ee323d06c0291b976c", status:"資料正常",
    description:"Top 10 高曝光頁面完整表格（含曝光、點擊、CTR、平均排名）。",
    filterKeys:[], chart:{type:"bar",xKey:"path",series:[["imp","曝光"]]},
    columns:[["rank","#"],["path","頁面路徑"],["imp","曝光"],["clk","點擊"],["ctr","CTR%"],["avg_pos","平均排名"]],
    rows:[
    {rank:1,path:"/",imp:7526,clk:3000,ctr:0,avg_pos:0},
    {rank:2,path:"/p/412-1033-4845.php",imp:2097,clk:993,ctr:0,avg_pos:0},
    {rank:3,path:"/p/412-1033-4832.php?Lang=zh-tw",imp:950,clk:505,ctr:0,avg_pos:0},
    {rank:4,path:"/p/412-1033-4858.php?Lang=zh-tw",imp:517,clk:366,ctr:0,avg_pos:0},
    {rank:5,path:"/p/412-1033-4858.php",imp:532,clk:341,ctr:0,avg_pos:0},
    {rank:6,path:"/p/412-1033-4801.php",imp:431,clk:317,ctr:0,avg_pos:0},
    {rank:7,path:"/p/412-1033-4801.php?Lang=zh-tw",imp:483,clk:301,ctr:0,avg_pos:0},
    {rank:8,path:"/p/412-1033-4832.php",imp:97,clk:77,ctr:0,avg_pos:0},
    {rank:9,path:"/p/412-1033-4846.php?Lang=zh-tw",imp:118,clk:77,ctr:0,avg_pos:0},
    {rank:10,path:"/p/426-1033-2.php?Lang=zh-tw",imp:61,clk:49,ctr:0,avg_pos:0}
  ],
  ...
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:d30a71164e40f9d985b040a96017f4f6d62475725b8128194e5bb2fa8b543707", status:"正常",
    ...
    {query:"www", cls:"nonbrand", imp:458, clicks:0, ctr:0, avg_pos:7.2, landing:"/"},
    ...
    {query:"慈濟大學中文版首頁", cls:"nonbrand", imp:102, clicks:2, ctr:1.96, avg_pos:4.3, landing:"/"},
    ...
    sql:"SELECT query, CASE WHEN REGEXP_CONTAINS(LOWER(query), r'慈濟|慈大|慈|tcu|tzu chi|www|資管|資訊科技與管理') THEN 'brand' ELSE 'nonbrand' END AS cls, SUM(impressions) AS imp, SUM(clicks) AS clicks, ROUND(100*SAFE_DIVIDE(SUM(clicks), SUM(impressions)), 2) AS ctr, ROUND(SUM(sum_position)/NULLIF(SUM(impressions), 0), 1) AS avg_pos FROM all_gsc_summary WHERE site_name='慈濟大學中文版首頁' AND data_date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND query IS NOT NULL AND query != '' GROUP BY query ORDER BY imp DESC LIMIT 50;"
  },
  {
    id:"EV-CONTENT-MATRIX-001", title:"內容效益矩陣完整報表", sourceType:"BigQuery",
    sourceTable:"all_units_summary", queryCode:"GA4-PAGE-MATRIX-001",
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:c9db99346f8e70ef802f70cb06dced5dba0b23bbddfc99c7a779e4183991f6a8", status:"正常",
    ...
    rows:[
    {normalized_path:"/p/426-1022-2.php?Lang=zh-tw", title:"智慧健康與生活管理學分學程專班 - 慈濟大學中文版首頁", users:34, sessions:34, pageviews:116, engagement_sec:0.5, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/index.php", title:"慈濟大學慈濟大學中文版首頁 - 慈濟大學中文版首頁", users:30, sessions:48, pageviews:172, engagement_sec:0.6, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/", title:"慈濟大學慈濟大學中文版首頁 - 慈濟大學中文版首頁", users:24, sessions:30, pageviews:100, engagement_sec:0.9, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/412-1022-2586.php", title:"師資陣容 - 慈濟大學中文版首頁", users:24, sessions:24, pageviews:70, engagement_sec:0.4, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/403-1022-452-1.php?Lang=zh-tw", title:"最新消息 - 慈濟大學中文版首頁", users:10, sessions:10, pageviews:24, engagement_sec:1.9, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/404-1022-52388.php?Lang=zh-tw", title:"碩士班專區 - 慈濟大學中文版首頁", users:9, sessions:11, pageviews:25, engagement_sec:0.3, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/404-1022-36099.php", title:"課程地圖與開課學分時數表 / Curriculum Map and Table of Courses Offered with Credit Hours - 慈濟大學中文版首頁", users:8, sessions:8, pageviews:50, engagement_sec:0.7, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/412-1022-2570.php?Lang=zh-tw", title:"最新消息 - 慈濟大學中文版首頁", users:8, sessions:8, pageviews:23, engagement_sec:1.1, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/404-1022-30026.php?Lang=zh-tw", title:"系所介紹 - 慈濟大學中文版首頁", users:7, sessions:8, pageviews:16, engagement_sec:0.4, median_users:7, median_eng:25, quadrant:"Q3"},
    {normalized_path:"/p/404-1022-34441.php", title:"課程規劃 - 慈濟大學中文版首頁", users:6, sessions:6, pageviews:16, engagement_sec:0.4, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/?Lang=zh-tw", title:"慈濟大學慈濟大學中文版首頁 - 慈濟大學中文版首頁", users:6, sessions:6, pageviews:11, engagement_sec:0.7, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/404-1022-30020.php", title:"入學方式 - 慈濟大學中文版首頁", users:6, sessions:6, pageviews:16, engagement_sec:0.3, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/426-1022-5.php?Lang=zh-tw", title:"智慧生活與數位轉型學分學程專班 - 慈濟大學中文版首頁", users:6, sessions:6, pageviews:13, engagement_sec:0.5, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/404-1022-30062.php", title:"規章辦法 - 慈濟大學中文版首頁", users:4, sessions:4, pageviews:10, engagement_sec:0.2, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/412-1022-2590.php?Lang=zh-tw", title:"業界實習 - 慈濟大學中文版首頁", users:4, sessions:4, pageviews:6, engagement_sec:2.7, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/403-1022-703-1.php?Lang=zh-tw", title:"Photo - 慈濟大學中文版首頁", users:4, sessions:4, pageviews:8, engagement_sec:0.6, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/404-1022-66044.php?Lang=zh-tw", title:"智慧物聯實驗室 - 慈濟大學中文版首頁", users:4, sessions:4, pageviews:6, engagement_sec:0.1, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/404-1022-60990.php?Lang=zh-tw", title:"碩士班相關表格 - 慈濟大學中文版首頁", users:4, sessions:4, pageviews:14, engagement_sec:0.7, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/404-1022-47588.php?Lang=zh-tw", title:"人工智慧高階電腦教室 - 慈濟大學中文版首頁", users:3, sessions:3, pageviews:9, engagement_sec:0.3, median_users:7, median_eng:25, quadrant:"Q4"},
    {normalized_path:"/p/17-1022.php?Lang=zh-tw", title:"網站導覽 - 慈濟大學中文版首頁", users:3, sessions:3, pageviews:4, engagement_sec:0.1, median_users:7, median_eng:25, quadrant:"Q4"}
  ],
    sql:"SELECT REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(page_location, r'\?.*$', ''), r'/$', ''), r'(\?|&)(utm_[^&]+|gclid=[^&]+|fbclid=[^&]+)', '') AS normalized_path, ANY_VALUE(page_title) AS title, COUNT(DISTINCT user_pseudo_id) AS users, COUNT(DISTINCT ga_session_id) AS sessions, COUNTIF(event_name='page_view') AS pageviews, ROUND(SAFE_DIVIDE(SUM(engagement_time_msec)/1000.0, COUNT(DISTINCT user_pseudo_id)), 1) AS engagement_sec FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND event_name='page_view' GROUP BY normalized_path ORDER BY users DESC LIMIT 50;"
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:980ecc0cb3c2f89f36bf38f07d07097e72cb7f3db372cbbda48e311cd46cbb78", status:"正常",
    ...
    rows:[
    {group:"direct",source:"(direct)",medium:"(none)",sessions:4679,users:2780,avg_eng_sec_per_session:0.1,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"organic",source:"google",medium:"organic",sessions:4340,users:2457,avg_eng_sec_per_session:0.2,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"organic",source:"bing",medium:"organic",sessions:177,users:97,avg_eng_sec_per_session:0.0,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"tcust.edu.tw",medium:"referral",sessions:105,users:65,avg_eng_sec_per_session:0.0,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"tw.search.yahoo.com",medium:"referral",sessions:78,users:46,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"organic",source:"baidu",medium:"organic",sessions:11,users:7,avg_eng_sec_per_session:1.2,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"jctv.ntut.edu.tw",medium:"referral",sessions:9,users:8,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"tcunew.tcust.edu.tw",medium:"referral",sessions:8,users:2,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"tzuchi.org.tw",medium:"referral",sessions:6,users:4,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"m.facebook.com",medium:"referral",sessions:5,users:5,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"ntp.msn.com",medium:"referral",sessions:5,users:1,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"足夠"},
    {group:"referral",source:"collego.edu.tw",medium:"referral",sessions:4,users:1,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"wwwc.moex.gov.tw",medium:"referral",sessions:4,users:4,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"m.baidu.com",medium:"referral",sessions:4,users:4,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"organic",source:"yahoo",medium:"organic",sessions:4,users:4,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"wifi.tcust.edu.tw:1000",medium:"referral",sessions:3,users:2,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"104.com.tw",medium:"referral",sessions:3,users:3,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"jingsi.org",medium:"referral",sessions:3,users:3,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"tw.tzuchi.org",medium:"referral",sessions:3,users:3,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"},
    {group:"referral",source:"cse.google.com",medium:"referral",sessions:3,users:1,avg_eng_sec_per_session:null,internal_clicks:0,downloads:0,cta_clicks:0,sample_note:"資料不足"}
  ],
    sql:"SELECT CASE WHEN source='(direct)' OR medium='(none)' THEN 'Direct' WHEN source='google' AND medium='organic' THEN 'Organic Search' WHEN REGEXP_CONTAINS(COALESCE(page_referrer,''), r'chatgpt|perplexity|gemini|copilot|claude|openai') THEN 'AI Referral' WHEN medium='referral' AND source LIKE '%edu%' OR source LIKE 'lle%' THEN 'Internal Referral' WHEN medium='referral' THEN 'External Referral' WHEN source='facebook.com' OR source='twitter.com' OR medium='social' THEN 'Social' ELSE 'Other' END AS group, source, medium, COUNT(DISTINCT ga_session_id) AS sessions, COUNT(DISTINCT user_pseudo_id) AS users, ROUND(SAFE_DIVIDE(SUM(engagement_time_msec)/1000.0, COUNT(DISTINCT ga_session_id)), 1) AS avg_eng_sec_per_session, SUM(CASE WHEN event_name='click' AND link_class IN ('internal-cta','internal-link') THEN 1 ELSE 0 END) AS internal_clicks, SUM(CASE WHEN link_url LIKE '%.pdf' OR link_url LIKE '%.doc%' OR link_url LIKE '%.xls%' THEN 1 ELSE 0 END) AS downloads, SUM(CASE WHEN event_name='click' AND link_class='admission-cta' THEN 1 ELSE 0 END) AS cta_clicks FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' GROUP BY group, source, medium ORDER BY sessions DESC;"
  },
  {
  ...
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:56e0a5f68664f18e54ca4560d7fb6a02acd738c038921fc0be7fef03cdae5dd1", status:"行動代理指標",
    ...
    {category:"其他連結", link_text:"null", source_page:"p/404-1022-60990.php?Lang=zh-tw", destination:"https://www.tcust.edu.tw/var/file/22/1022/img/1843/540712717.doc", clicks:6, users:2, valid_events:0, valid_rate:0},
    ...
    {category:"其他連結", link_text:"下載", source_page:"p/404-1022-36099.php", destination:"https://www.tcust.edu.tw/var/file/22/1022/img/1214/686385563.ods", clicks:5, users:3, valid_events:0, valid_rate:0},
    ...
    {category:"其他連結", link_text:"null", source_page:"p/404-1022-36099.php", destination:"https://www.tcust.edu.tw/var/file/22/1022/img/1214/686385563.ods", clicks:5, users:3, valid_events:0, valid_rate:0},
    {category:"其他連結", link_text:"慈濟大學中文版首頁碩士班-資管系碩士論文格式", source_page:"p/404-1022-60990.php?Lang=zh-tw", destination:"https://www.tcust.edu.tw/var/file/22/1022/img/1843/540712717.doc", clicks:5, users:2, valid_events:0, valid_rate:0},
    {category:"其他連結", link_text:"第三人生大學", source_page:"p/426-1022-2.php?Lang=zh-tw", destination:"https://www.tcu.edu.tw/p/426-1022-2.php?Lang=zh-tw", clicks:4, users:3, valid_events:4, valid_rate:1},
    {category:"其他連結", link_text:"下載", source_page:"p/404-1022-36099.php", destination:"https://www.tcu.edu.tw/var/file/22/1022/attach/15/pta_90538_1108797_18692.pdf", clicks:4, users:3, valid_events:4, valid_rate:1},
    {category:"其他連結", link_text:"上課資訊", source_page:"p/426-1022-2.php?Lang=zh-tw", destination:"https://www.tcu.edu.tw/p/426-1022-2.php?Lang=zh-tw#schedule", clicks:4, users:4, valid_events:4, valid_rate:1},
    {category:"其他連結", link_text:"課程特色", source_page:"p/426-1022-2.php?Lang=zh-tw", destination:"https://www.tcu.edu.tw/p/426-1022-2.php?Lang=zh-tw#courses", clicks:3, users:2, valid_events:3, valid_rate:1},
    {category:"其他連結", link_text:"入學方式", source_page:"", destination:"https://www.tcu.edu.tw/p/404-1022-30020.php", clicks:3, users:3, valid_events:3, valid_rate:1}
  ],
    sql:"SELECT CASE WHEN link_url LIKE 'line.me%' OR link_url LIKE '%line.me%' THEN 'LINE' WHEN link_url LIKE 'tel:%' THEN '電話' WHEN link_url LIKE 'mailto:%' THEN 'Email' WHEN link_url LIKE '%.pdf' OR link_url LIKE '%.doc%' OR link_url LIKE '%.xls%' THEN '下載文件' WHEN (page_title LIKE '%招生%' OR link_text LIKE '%招生%') AND (link_url LIKE '%adm%' OR link_url LIKE '%recruit%' OR link_url LIKE '%p/16%' OR link_url LIKE '%p/406%') THEN '招生簡章' WHEN (page_title LIKE '%入學%' OR link_text LIKE '%入學%') THEN '入學方式' WHEN link_url LIKE '%apply%' OR link_text LIKE '%報名%' THEN '報名系統' ELSE '其他連結' END AS category, ANY_VALUE(link_text) AS link_text, ANY_VALUE(page_title) AS source_page, link_url AS destination, COUNT(*) AS clicks, COUNT(DISTINCT user_pseudo_id) AS users, 1.0 AS valid_rate FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND event_name='click' AND link_url IS NOT NULL AND link_url != '' GROUP BY category, destination ORDER BY clicks DESC LIMIT 50;"
  },
  {
  ...
  },
  {
    ...
    period:"{{PREVIOUS_START}} 至 {{PREVIOUS_END}}（7 天 daily, 1 minggu ke belakang）", maxDate:"{{PREVIOUS_END}}", generatedAt:"2026-08-03T14:00:00Z",
    jobId:"not captured", dataHash:"sha256:8303142c9841bb04fad93a8867264d1934bb4d130e8b4e72b6d93182d179b058", status:"路徑代理",
    description:"依 page_view 事件彙總的「入口頁 → 第二頁」共同出現次數（{{PREVIOUS_START}} 至 {{PREVIOUS_END}} 期間，5 個完整天）。本期資料以 ROW_NUMBER() OVER (PARTITION BY session ORDER BY event_timestamp) 重建單一工作階段內的時序。Refreshed 2026-07-31T12:11:40Z：本期 150/177 工作階段僅有 1 個 page_view，入口頁和第二頁多為同一頁（self-loop），故右側「入口頁重點轉換」表全為 0。",
    filterKeys:["landing"], chart:{type:"bar",xKey:"landing",series:[["sessions","工作階段"]]},
    columns:[["from_path","來源頁"],["to_path","到達頁"],["transitions","轉場數"]],
    rows:[
    {from_path:"/",to_path:"/p/412-1033-4845.php",transitions:844},
    {from_path:"/",to_path:"/p/412-1033-4832.php?Lang=zh-tw",transitions:612},
    {from_path:"/",to_path:"/p/412-1033-4801.php?Lang=zh-tw",transitions:267},
    {from_path:"/",to_path:"/p/412-1033-4801.php",transitions:183},
    {from_path:"/p/412-1033-4845.php",to_path:"/",transitions:171},
    {from_path:"/p/412-1033-4832.php?Lang=zh-tw",to_path:"/",transitions:143},
    {from_path:"/",to_path:"/p/412-1033-4858.php?Lang=zh-tw",transitions:139},
    {from_path:"/",to_path:"/p/412-1033-4846.php?Lang=zh-tw",transitions:69},
    {from_path:"/p/412-1033-4801.php?Lang=zh-tw",to_path:"/",transitions:68},
    {from_path:"/p/412-1033-4858.php?Lang=zh-tw",to_path:"/",transitions:50},
    {from_path:"/p/412-1033-4801.php",to_path:"/",transitions:48},
    {from_path:"/p/412-1033-4801.php?Lang=zh-tw",to_path:"/p/412-1033-4832.php?Lang=zh-tw",transitions:39},
    {from_path:"/p/412-1033-4832.php?Lang=zh-tw",to_path:"/p/412-1033-4801.php?Lang=zh-tw",transitions:37},
    {from_path:"/",to_path:"/p/412-1033-4828.php",transitions:33},
    {from_path:"/p/412-1033-4845.php",to_path:"/p/412-1033-4858.php?Lang=zh-tw",transitions:32}
  ],
    sql:"SELECT first_page AS landing, second_page, '瀏覽後離開' AS exit_action, COUNT(DISTINCT ga_session_id) AS sessions, ROUND(SAFE_DIVIDE(COUNT(*), SUM(COUNT(*)) OVER (PARTITION BY first_page)), 2) AS exit_share FROM (SELECT ga_session_id, MIN_BY(page_title, event_timestamp) AS first_page, NTH_VALUE(page_title, 2) OVER (PARTITION BY ga_session_id ORDER BY event_timestamp ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) AS second_page FROM all_units_summary WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' AND event_name='page_view' GROUP BY ga_session_id, page_title, event_timestamp) GROUP BY first_page, second_page ORDER BY sessions DESC LIMIT 50;"
  },
  {
    ...
    sql:"SELECT 'missing_jsonld' AS issue_type, COUNTIF(REGEXP_CONTAINS(body, r'application/ld\+json')) AS affected_count FROM `project.all_units_summary` WHERE site_name='慈濟大學中文版首頁' AND date BETWEEN DATE '{{PREVIOUS_START}}' AND DATE '{{PREVIOUS_END}}' UNION ALL /* … */ ;"
  }
];
